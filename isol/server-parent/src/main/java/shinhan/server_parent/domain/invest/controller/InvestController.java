...
public class InvestController {
    ...
    InvestController(InvestService investService, JwtService jwtService,
        AccountUtils accountUtils) {
    ...
    public ApiUtils.ApiResult getInvestHistory(@PathVariable("status") short status,
        ...
        Account accountByUserSerialNumberAndStatus = accountUtils.getAccountByUserSerialNumberAndStatus(
            csn, 2);
        return success(
            investService.getStockHistory(accountByUserSerialNumberAndStatus.getAccountNum(),
                status, year, month));
    }

    //포트폴리오 조회하기(부모)
    @GetMapping("/portfolio")
    public ApiUtils.ApiResult getInvestPortfolio(@RequestParam("csn") Long csn)
        throws AuthException {
            if (!jwtService.isMyFamily(csn)) {
                throw new CustomException(ErrorCode.FAILED_NO_CHILD);
            }
            ...
            Account accountByUserSerialNumberAndStatus = accountUtils.getAccountByUserSerialNumberAndStatus(
                csn, 2);
            PortfolioResponse result = investService.getPortfolio(
                accountByUserSerialNumberAndStatus.getAccountNum());
