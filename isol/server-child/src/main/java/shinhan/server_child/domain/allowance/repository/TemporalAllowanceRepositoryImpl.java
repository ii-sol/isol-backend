...
public class TemporalAllowanceRepositoryImpl implements TemporalAllowanceRepositoryCustom {
    ...
    public List<TemporalAllowance> findByUserSerialNumberAndCreateDate(Long userSerialNumber, Integer year, Integer month, Integer csn){
    ...
    public List<TemporalAllowance> findByChildSerialNumberAndCreateDateAndStatus(Long userSerialNumber, Integer year, Integer month){
        String jpql = "select t from TemporalAllowance t where t.child.serialNumber = :serialNumber and t.status IN (4, 5, 6) and FUNCTION('MONTH', t.createDate) = :month AND FUNCTION('YEAR', t.createDate) = :year";
